...
public class PieChartActivity extends AppCompatActivity implements OnChartValueSelectedListener {
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView((int) R.layout.activity_pie_chart);
        ...
        this.toolbar_chart.setNavigationOnClickListener(new OnClickListener() {
        ...
        try {
            this.catList = this.dataManager.getCategory();
        ...
        if (this.expenseList != null) {
            for (int i = 0; i < this.catList.size(); i++) {
                this.catIdExpense = ((Category) this.catList.get(i)).getCatId();
                String str = "";
                Double valueOf = Double.valueOf(0.0d);
                int i2 = 0;
                for (int i3 = 0; i3 < this.expenseList.size(); i3++) {
                    if (this.catIdExpense == Integer.parseInt(((Expense) this.expenseList.get(i3)).getCategoryId())) {
                        i2++;
                        Double.valueOf(0.0d);
                        Double valueOf2 = Double.valueOf(((Expense) this.expenseList.get(i3)).getAmount());
                        if (valueOf.doubleValue() != 0.0d) {
                            valueOf2 = Double.valueOf(valueOf.doubleValue() + valueOf2.doubleValue());
                        }
                        Double d = valueOf2;
                        str = ((Expense) this.expenseList.get(i3)).getCategory();
                        valueOf = d;
                    }
                }
                if (i2 > 0) {
